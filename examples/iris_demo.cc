...
double evaluate(MLP &mlp, const CryptoContext<DCRTPoly> &cc,
  ...
  std::vector<std::vector<int>> conf(num_classes,
  ...
  for (int i = 0; i < total; i++) {
    try {
      ...
      int pred =
          (int)std::distance(v.begin(), std::max_element(v.begin(), v.end()));
      int label = (int)Y[i];
  ...
  std::cout << "\n=== Per Class ===" << std::endl;
  std::cout << std::setw(8) << "class" << std::setw(10) << "precision"
  ...
  for (int c = 0; c < num_classes; c++) {
    ...
    std::cout << std::setw(8) << c << std::setw(9) << std::fixed
              << std::setprecision(1) << precision << "%" << std::setw(9)
  ...
  std::cout << "Accuracy:  " << correct << "/" << evaluated << " = " << accuracy
            << "%" << std::endl;
...
template <typename T> void load_data(T &data, const std::string &filename) {
  std::string buffer;
