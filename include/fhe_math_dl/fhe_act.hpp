...
inline int log2_int(unsigned int n) { return n ? 31 - __builtin_clz(n) : -1; }

////////////SIGMOID////////////
// σ(x) = 1 / (1 + exp(-x)) //
/////////////////////////////
template <size_t N>
Ciphertext<DCRTPoly> polynomial_sigmoid_fhe(const Ciphertext<DCRTPoly> &x,
  ...
  for (int p = 3; p <= max_deg; p += 2) {
    int k = log2_int(p);
    int rem = p - (1 << k);
    powMap[p] = (rem == 1) ? cc->EvalMult(evenPow[k - 1], ct_in)
...
template <size_t N>
Ciphertext<DCRTPoly>
silu_fhe(const Ciphertext<DCRTPoly> &x, const CryptoContext<DCRTPoly> &cc,
         const std::array<double, N> &coeffs, int valid_slots = -1) {
