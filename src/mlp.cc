...
void MLP::add_layer(const std::vector<std::vector<double>> &W,
...
Ciphertext<DCRTPoly> MLP::linear_layer(const Ciphertext<DCRTPoly> &ct_input,
  ...
  std::vector<std::vector<double>> diags(layer.in_dim,
                                         std::vector<double>(layer.out_dim));
  ...
  for (int k = 0; k < layer.in_dim; k++) {
    ...
    if (first) {
      ...
      ct_result = cc->EvalAdd(ct_result, ct_mul);
  }
  ...
  for (auto &[key, value] : obj) {
    ...
    if (!arr.empty() && arr[0].holds<glz::json_t::array_t>()) {
      ...
      for (size_t i = 0; i < arr.size(); ++i) {
        auto &row = arr[i].get<glz::json_t::array_t>();
...
Ciphertext<DCRTPoly> MLP::forward(const Ciphertext<DCRTPoly> &input,
                                  const CryptoContext<DCRTPoly> &cc) {
